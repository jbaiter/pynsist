...
def fetch_python(version=DEFAULT_PY_VERSION, bitness=DEFAULT_BITNESS,
    ...
    url = 'http://python.org/ftp/python/{0}/python-{0}{1}.msi'.format(version,
                                                                      arch_tag)
    ...
    try:
        ...
        logger.warn("GPG not available - could not check signature of {0}"
                    .format(target))
...
def write_script(entrypt, python_version, bitness, target):
    ...
    with open(target, 'w') as f:
        f.write(SCRIPT_TEMPLATE.format(qualifier=qualifier, module=module,
                                       func=func))
...
def run_nsis(nsi_file):
    ...
    try:
        if os.name == 'nt':
            makensis = pjoin(winreg.QueryValue(winreg.HKEY_LOCAL_MACHINE,
                                               'SOFTWARE\\NSIS'),
                             'makensis.exe')
        else:
            makensis = 'makensis'
...
def all_steps(appname, version, script=None, entry_point=None,
              icon=DEFAULT_ICON, console=False, packages=None,
    ...
    if entry_point is not None:
        ...
        script = 'launch.py'
        write_script(entry_point, py_version, py_bitness, pjoin(build_dir,
                                                                script))
    ...
    if not exitcode:
        logger.info('Installer written to %s', pjoin(build_dir,
                                                     installer_name))
...
def main(argv=None):
    ...
    all_steps(
        ...
        nsi_template=cfg.get('Build', 'nsi_template',
                             fallback=DEFAULT_NSI_TEMPLATE),
    )
