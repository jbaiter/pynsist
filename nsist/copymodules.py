...
def copy_modules(modnames, target, path=None):
    ...
    By default, it finds modules in sys.path - this can be overridden by
    passing the path parameter.
    """
    mc = ModuleCopier(path)
    files_in_target_noext = [os.path.splitext(f)[0]
    ...
    if not modnames:
        ...
        with open(os.path.join(target, 'placeholder'), 'w') as f:
            f.write('This file only exists so NSIS finds something in this '
                    'directory.')
